...
public class UpdateProfessorWorker extends Worker {
    private final String TAG = "UpdateProfessorWorker";
    ...
    public Result doWork() {
        ...
        String senha = util.getPassword();
        String topicoExclusivo = util.getTopico();
        JSONObject requestParams = new JSONObject();
        try {
            requestParams.put("nome", nome);
            requestParams.put("email", email);
            requestParams.put("topicoPrivado", topicoExclusivo);
            requestParams.put("senha", senha);
        } catch (JSONException e) {
            e.printStackTrace();
            return Result.RETRY;
        }
        String url = getApplicationContext().getString(R.string.server_url) + "/api/professor/" + util.getMatricula() + "/update";

...
public class NoBlocoAction extends BroadcastReceiver implements ResponseListener<JSONObject> {
    private static final String TAG = "NoBlocoAction";
    ...
    private void mandarNotificacao(Context context) {
        ...
        CheckinTask mTask = new CheckinTask(context, this);
        mTask.execute();
    }
    ...
    public void doThis(JSONObject response) {
        if(response == null)
            Log.e(TAG, "doThis: resposta é nula", new NullPointerException());
        else
            Log.d(TAG, "doThis: Enviado ao servidor com sucesso");
    }

    class CheckinTask extends AsyncTask<Void, Void, JSONObject>{
        Context mContext;
        ResponseListener<JSONObject> mListener;
        public CheckinTask(Context context, ResponseListener<JSONObject> listener){
            mContext = context;
            mListener = listener;
        }
